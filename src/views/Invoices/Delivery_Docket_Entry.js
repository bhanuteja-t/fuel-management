...
import { createdocket, getsiteorsupplier } from '../../Apicalls';
...
function Delivery_Docket_Entry() {
    ...
    const [errors] = useState({
        supplierErr: "Select Supplier",
        DocketNoErr: "Enter Docket Number",
        TempDocketNoErr: "Provide Temp Docket Num",
        DateErr: "Enter Date",
    });
    const {
        supplierErr,
    ...
    const isvalid = () => {
        if (
            ...
            toast.error(DocketNoErr, { position: toast.POSITION.TOP_RIGHT });
            return false;
        }
        ...
        else if (!TempDocketNo.length > 0) {
            toast.error(TempDocketNoErr, { position: toast.POSITION.TOP_RIGHT });
            return false;
        }
        return true;
    }
    ...
    const listofsupp = listofsuppliers.map((obj) => ({
        key: obj.name,
        text: obj.name,
        value: obj.name
    }));
